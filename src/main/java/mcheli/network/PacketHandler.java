...
public class PacketHandler extends MessageToMessageCodec<FMLProxyPacket, PacketBase> {
    ...
    public boolean registerPacket(Class<? extends PacketBase> cl) {
        if (packets.size() > 256) {
            System.out.println("Packet limit exceeded in Flan's Mod packet handler by packet " + cl.getCanonicalName() + ".");
            return false;
        }
        if (packets.contains(cl)) {
            System.out.println("Tried to register " + cl.getCanonicalName() + " packet class twice.");
            return false;
        }
        if (modInitialised) {
            System.out.println("Tried to register packet " + cl.getCanonicalName() + " after mod initialisation.");
            return false;
        }
    ...
    public void initialise() {
        ...
        registerPacket(PacketCountermeasureState.class);
        registerPacket(PacketDamageIndicator.class);
        registerPacket(PacketECMJammerUse.class);
        registerPacket(PacketEntityInfoSync.class);
        registerPacket(PacketGPSPositionReset.class);
        registerPacket(PacketIronCurtainUse.class);
        registerPacket(PacketLaserGuidanceTargeting.class);
        registerPacket(PacketLaserStateSync.class);
        registerPacket(PacketLockTarget.class);
        registerPacket(PacketLockTargetBVR.class);
        registerPacket(PacketMissileLockType.class);
        registerPacket(PacketRadarLockState.class);
        registerPacket(PacketRadarSwitchState.class);
        registerPacket(PacketRWRThreatSync.class);
