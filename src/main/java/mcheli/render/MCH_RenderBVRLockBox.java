...
// Track the "best" hard-lock target this frame (smallest angle, then nearest)
        ...
        int sw = sc.getScaledWidth();
        int sh = sc.getScaledHeight();
        ...
        for (MCH_EntityInfo entity : entities) {
            ...
            // Behind camera / failed projection
            if (sx < 0 || sy < 0) continue;

            // (Optional) if you only want boxes when on-screen, keep this.
            // If you want clamping-to-edge, remove this and clamp sx/sy instead.
            if (sx < 0 || sx > sw || sy < 0 || sy > sh) continue;

            double dx = gx - ac.posX;
            double dy = gy - ac.posY;
            double dz = gz - ac.posZ;
            ...
            String text = String.format("[%s %.1fm]", rwrResult.name, dist);
            int color = isMSL ? 0xFF0000 : (lock ? 0xFF0000 : 0x00FF00);
            ...
            // Draw the box at screen position
            drawTexturedQuad2D(sx, sy, BOX_SIZE);

            if (drawText) {
                int fw = mc.fontRenderer.getStringWidth(text);
                mc.fontRenderer.drawString(text, (int) (sx - fw / 2), (int) (sy + BOX_SIZE * 0.5f + 2), color, false);
            }
        }
