...
public class MCH_EntityThrowable extends EntityThrowable {
    ...
    public MCH_EntityThrowable(World par1World, EntityLivingBase par2EntityLivingBase, float acceleration) {
        ...
        super.motionX *= acceleration;
        super.motionY *= acceleration;
        super.motionZ *= acceleration;
        this.init();
    }
    ...
    public MCH_EntityThrowable(World p_i1777_1_, double x, double y, double z, float yaw, float pitch) {
        ...
        super.yOffset = 0.0F;
        float f = 0.4F;
        super.motionX = -MathHelper.sin(super.rotationYaw / 180.0F * 3.1415927F) * MathHelper.cos(super.rotationPitch / 180.0F * 3.1415927F) * f;
        super.motionZ = MathHelper.cos(super.rotationYaw / 180.0F * 3.1415927F) * MathHelper.cos(super.rotationPitch / 180.0F * 3.1415927F) * f;
        super.motionY = -MathHelper.sin((super.rotationPitch + this.func_70183_g()) / 180.0F * 3.1415927F) * f;
    ...
    public void init() {
        ...
        this.setInfo(null);
        this.noInfoCount = 0;
        this.getDataWatcher().addObject(31, "");
    }

    public void setDead() {
        String s = this.getInfo() != null ? this.getInfo().name : "null";
        MCH_Lib.DbgLog(super.worldObj, "MCH_EntityThrowable.setDead(%s)", s);
        super.setDead();
    }
    ...
    public void onUpdate() {
        ...
        if (this.getInfo() != null) {
            ...
            if (i != null && y == Material.water) {
                super.motionY += this.getInfo().gravityInWater;
            } else {
                super.motionY += this.getInfo().gravity;
            }
        }
    ...
    public void setInfo(MCH_ThrowableInfo info) {
        ...
        if (info != null && !super.worldObj.isRemote) {
            this.getDataWatcher().updateObject(31, info.name);
        }
