...
public class MCP_ClientPlaneTickHandler extends MCH_AircraftClientTickHandler {
    ...
    protected void onTick(boolean inGUI) {
        ...
        for (MCH_Key viewEntityDummy : player) {
        ...
        if (!super.isBeforeRiding && super.isRiding) {
    ...
    protected void playerControl(EntityPlayer player, MCP_EntityPlane plane, boolean isPilot) {
        ...
        if (!plane.isDestroyed() && this.KeyAirburstDistReset.isKeyDown()) {
            if (plane.getSeatIdByEntity(player) <= 1) {
                if (plane.getCurrentWeapon(player) != null) {
                    MCH_WeaponSet ws = plane.getCurrentWeapon(player);
                    if (ws.getCurrentWeapon() != null) {
                        MCH_WeaponBase wb = ws.getCurrentWeapon();
                        if (wb.getInfo() != null && wb.getInfo().canAirburst) {
                            plane.resetAirburstDistance(player, wb);
                            playSoundOK();
                        }
                    }
                }
