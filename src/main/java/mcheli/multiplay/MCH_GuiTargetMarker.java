...
public class MCH_GuiTargetMarker extends MCH_Gui {
    ...
    public static void addMarkEntityPos(int reserve, Entity entity, double x, double y, double z, boolean nazo) {
        if (isEnableEntityMarker()) {
            ...
            if (MCH_FMURUtil.isSoldier(entity)) {
                Team t = MCH_FMURUtil.getSoldierTeam(entity);
                if (t == null) {
                    return;
                }
                if (clientPlayer.isOnTeam(t)) {
                    spotType = MCH_TargetType.SAME_TEAM_PLAYER;
                }
            }
    ...
    public static boolean isEnableEntityMarker() {
        if (MCH_Config.DisplayEntityMarker.prmBool && (Minecraft.getMinecraft().isSingleplayer() || MCH_ServerSettings.enableEntityMarker)) {
            return MCH_Config.EntityMarkerSize.prmDouble > 0.0D;
        }
        return false;
    }
    ...
    void drawMark() {
        ...
        for (int i = 0; i < 2; ++i) {
            ...
            for (Object entityPo : entityPos) {
                MCH_MarkEntityPos e = (MCH_MarkEntityPos) entityPo;
                ...
                if (i == 0) {
                    ...
                    if (z < 1.0D && x >= 0.0D && x <= (double) (DSW - 20) && y >= 0.0D && y <= (double) (DSH - 40)) {
                        S = super.mc.thePlayer.getDistanceToEntity(e.entity);
                        GL11.glEnable(3553);
                        ...
                        if (x >= (double) (DSW / 2 - 20) && x <= (double) (DSW / 2 + 20) && y >= (double) (DSH / 2 - 20) && y <= (double) (DSH / 2 + 20)) {
                            this.drawString(String.format("x : %.0f", e.entity.posX), (int) (x + MARK_SIZE + 18.0D), (int) y - 12, color);
                            this.drawString(String.format("y : %.0f", e.entity.posY), (int) (x + MARK_SIZE + 18.0D), (int) y - 4, color);
                            this.drawString(String.format("z : %.0f", e.entity.posZ), (int) (x + MARK_SIZE + 18.0D), (int) y + 4, color);
                        }
    ...
    public void drawTriangle1(Tessellator tessellator, double x, double y, double size, int color) {
        tessellator.setColorRGBA_I(16777215 & color, color >> 24 & 255);
        tessellator.addVertex(x + size / 2.0D, y - 10.0D - size, super.zLevel);
        tessellator.addVertex(x - size / 2.0D, y - 10.0D - size, super.zLevel);
        tessellator.addVertex(x + 0.0D, y - 10.0D, super.zLevel);
    }
