...
public class MCH_ClientTankTickHandler extends MCH_AircraftClientTickHandler {
    ...
    protected void playerControl(EntityPlayer player, MCH_EntityTank tank, boolean isPilot) {
        ...
        if (!tank.isDestroyed() && this.KeyAirburstDistReset.isKeyDown()) {
            if (tank.getSeatIdByEntity(player) <= 1) {
                if (tank.getCurrentWeapon(player) != null) {
                    MCH_WeaponSet ws = tank.getCurrentWeapon(player);
                    if (ws.getCurrentWeapon() != null) {
                        MCH_WeaponBase wb = ws.getCurrentWeapon();
                        if (wb.getInfo() != null && wb.getInfo().canAirburst) {
                            tank.resetAirburstDistance(player, wb);
                            playSoundOK();
                        }
                    }
                }
