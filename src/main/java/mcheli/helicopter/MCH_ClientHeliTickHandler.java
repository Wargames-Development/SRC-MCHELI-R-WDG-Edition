...
public class MCH_ClientHeliTickHandler extends MCH_AircraftClientTickHandler {
    ...
    protected void playerControl(EntityPlayer player, MCH_EntityHeli heli, boolean isPilot) {
        ...
        if (!heli.isDestroyed() && this.KeyAirburstDistReset.isKeyDown()) {
            if (heli.getSeatIdByEntity(player) <= 1) {
                if (heli.getCurrentWeapon(player) != null) {
                    MCH_WeaponSet ws = heli.getCurrentWeapon(player);
                    if (ws.getCurrentWeapon() != null) {
                        MCH_WeaponBase wb = ws.getCurrentWeapon();
                        if (wb.getInfo() != null && wb.getInfo().canAirburst) {
                            heli.resetAirburstDistance(player, wb);
                            playSoundOK();
                        }
                    }
                }
