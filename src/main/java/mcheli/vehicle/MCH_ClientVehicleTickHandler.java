...
public class MCH_ClientVehicleTickHandler extends MCH_AircraftClientTickHandler {
    ...
    protected void playerControl(EntityPlayer player, MCH_EntityVehicle vehicle, boolean isPilot) {
        ...
        if (!vehicle.isDestroyed() && this.KeyAirburstDistReset.isKeyDown()) {
            if (vehicle.getSeatIdByEntity(player) <= 1) {
                if (vehicle.getCurrentWeapon(player) != null) {
                    ...
                    if (ws.getCurrentWeapon() != null) {
                        ...
                        if (wb.getInfo() != null && wb.getInfo().canAirburst) {
                            vehicle.resetAirburstDistance(player, wb);
                            playSoundOK();
                        }
                    }
                }
