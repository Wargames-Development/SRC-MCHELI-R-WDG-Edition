...
- 新概率：
  ...
  - 结果再 `clamp(0,1)`（即命中概率大于 1 时按 1 计算）

### 3.3 兼容策略
- 默认值改为 `near=1.5, far=0.5`。
- 老配置未写 `RadarGainFactor` 时，仍可稳定运行；概率上限由 `clamp(0,1)` 保护，不会出现“配置炸裂”。
- 该默认值可提高近距离对低 RCS 目标的发现概率，用于体现“烧穿距离”特征。
...
- 对比 `GainFactor=1.0,0.5` 与 `1.5,0.5`，确认近距提升明显、远距维持衰减趋势。
