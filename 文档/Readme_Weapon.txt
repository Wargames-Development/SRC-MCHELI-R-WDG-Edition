...
;AHEAD=true 时，此参数作为“空炸提前距离”使用，且仅在火控已解算有效空炸距离时生效
;AHEAD=true 且未建立有效火控解算时，不触发该近炸
;近炸默认可对载具目标生效；对导弹目标仅限“当前已锁定目标导弹”可触发（未锁定导弹不触发）
;主动雷达弹若已自主捕获目标导弹，同样可触发该近炸
...
;说明：
;1) 以上参数在 isGPSMissile=true 时生效（lockEntity=false/true 两种GPS链路均可用），目前有bug，属于废案
